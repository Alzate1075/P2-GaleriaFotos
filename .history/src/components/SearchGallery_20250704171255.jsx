...
const fotosFil = fotos.filter(
  fotos => fotos.title.tolowerCase().includes(buscarT.tolowerCase())
)

useEffect(() => {
  axios.get(`https://api.pexels.com/v1/search?query=mountain&per_page=12`)
  .then((response) => {
    setFotos(response.data.products)
  })
})
.catch((error)=> {
  console.error(error)
